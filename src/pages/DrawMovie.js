...
const DrawMovie = ()=>{
    const randomNumber = Math.floor(Math.random()*725801)
    const {movie,video,actors, isLoading} = useSelector((state)=>state.detail)
    const dispatch = useDispatch()
    useEffect(()=>{
        dispatch(loadDetail(randomNumber))
        const year = movie.release_date.split('-');
        if(movie.release_date == undefined){
            return randomNumber+1
        }
        else{
            if(year[0] < 2000 || year[0]==undefined){
                return randomNumber+1
            }
            if(year[0] == null){
               return randomNumber+1
...
console.log(randomNumber)
    const finalImage = `https://image.tmdb.org/t/p/original/${movie.backdrop_path}`;
